...
CEvents* CEventList::AddEntry(CEvents* events) REENTRANT({
    ...
    int CEventList::GetNextTimeout(msec& sendTime) REENTRANT({
        ...
        while (msgEltPtr)
        {
            ...
            {
                if (sendTime > tmpTime) sendTime = tmpTime;
            }
...
int CEventList::HandleEvents(const struct pollfd* readfds, const int fds)
{
    ...
    return status;
        ...
        void CEventList::GetFdSets(int& maxfds, fd_set& readfds,
            fd_set& writefds, fd_set& exceptfds) REENTRANT({
            CEventListElt* msgEltPtr = m_head.GetNext();
            ...
            while (msgEltPtr)
            {
                if (msgEltPtr->GetEvents()->GetCount())
                {
                    msgEltPtr->GetEvents()->GetFdSets(
                        maxfds, readfds, writefds, exceptfds);
                }
                msgEltPtr = msgEltPtr->GetNext();
            }
        })
                ...
                const fd_set& exceptfds)
{
    ...
    while (msgEltPtr)
    {
        ...
        {
            unlock();
            status = msgEltPtr->GetEvents()->HandleEvents(
                maxfds, readfds, writefds, exceptfds);
            lock();
        }
        msgEltPtr = msgEltPtr->GetNext();
